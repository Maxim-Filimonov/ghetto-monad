...
function errorOrObject(jsonString: string): Either<ErrorResult<Error>, Result<object>> {
    try {
...
describe('Either', () => {
    it('Returns an error', () => {
        ...
        expect((result as ErrorResult<Error>).error instanceof Error).toBe(true);
    });
    it('Returns an Result', () => {
        ...
        expect((result as ErrorResult<Error>).error instanceof Error).toBe(false);
    });
});
